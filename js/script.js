...
$(document).ready(function() {
  ...
  $("#search-submit").click(function() {
    ...
    if (subj || year || skills) {
      ...
      alert(
        "Please enter at least one of Subject Area, Year Level, and Skills!"
      );
    }
  });
...
function initSkillProfile() {
  ...
  $.each($(".courses-body"), function() {
    if (
      ...
      $(this)
        .find("ul")
        .append("<li>No courses applied to this skill.</li>");
    }
  });
...
function search(searchSubj, searchYear, searchSkills) {
  ...
  for (var i in courses) {
    ...
    $("#results-table")
      .find("tbody")
      .append(courseRow(code, course, skills));
  }
...
function courseRow(code, course, skills) {
  ...
  for (var i in skills) {
    skillLabels +=
      "<span class='label label-pill label-primary skill-label'>" +
      toTitleCase(skills[i]["skill"]) +
      " +" +
      skills[i]["points"] +
      "</span>";
  }
  ...
  var preBtn =
    "<input type='button' class='btn btn-default btn-xs btn-actions btn-pre'" +
    "value='Preview Skill Profile' onclick='previewCourse(this)' />";
  var addBtn =
    "<input type='button' class='btn btn-default btn-xs btn-actions btn-add'" +
    "value='Add to Worklist' onclick='addCourse(this)' />";
  var sscBtn =
    "<input type='button' class='btn btn-default btn-xs btn-actions btn-ssc'" +
...
function previewSkillProfile(course) {
  ...
  $.each(skills, function(skill, points) {
    ...
    var previewBar =
      "<div class='progress-bar progress-bar-striped progress-bar-danger active preview-bar'" +
      "aria-valuemin='0' aria-valuemax='120' aria-valuenow='" +
      points +
      "'" +
      "style='width: " +
      (points / 120) * 100 +
...
function addCourse(addBtn) {
  ...
  var confirmationBody =
    ...
    course +
    "</b>" +
    " has been successfully added to your worklist." +
    "<br>" +
...
function toTitleCase(string) {
  return string.replace(/-/g, " ").replace(/\w\S*/g, function(x) {
